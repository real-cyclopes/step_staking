...
export const formatWithCommas = (numberStr: string) => {
  return numberStr.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
};

export function getTokenBalanceStr(balance: number) {
  return NumberFormat.format(+balance);
}

export function getUsdPriceStr(price: number, minValue: number) {
  const absPrice = Math.abs(price);

  if (absPrice < minValue) {
    return `<  $${minValue}`;
  }

  return `$${NumberFormat.format(+price.toFixed(Math.max(1, 2 - (absPrice > 1 ? Math.floor(Math.log10(absPrice)) : 0))))}`;
}
